...
//      b. Write at Index Mode (code: 110) (on next input). Successive byte inputs increment index
...
//      b. Write at Index (code: 1010) (on next input). Successive byte inputs increment index
//  7. Key Bytes Index (5 bits) (types_pkg::chacha_key_index_t)
//      a. Read (code: 1011)
//      b. Write Mode (code: 1100) (on next inputs)
//  8. Block Counter (64 bits)
//      a. Read (code: 1101)
//      b. Write at Index (code: 1110) (on next input) (Note: This writes
//      from LSB to MSB)
//  9. Block Counter Index (3 bits)
//      a. Read (code: 1111)
//      b. Write Mode (code: 1'0000) (on next input). Successive byte inputs increment index
//  10. Start Hashing (code: 1'0001)
//  11. Reset Hash (code: 1'0010)
//  12. Read if Hash has Started (code: 1'0011)
//  13. Read Mode (code: 1'0100) (Read Mode literally means read what the
//  current mode is)
...
// 5. Nonce Bytes Index Setup Mode
// 6. Key Bytes Input Mode
// 7. Key Bytes Index Setup Mode
// 8. Block Counter Input Mode
// 9. Block Counter Index Setup Mode
...
module command_center (
    ...
    // Sent to Hash Generator (condition: command CMD_IV_STD_READ)
    ...
    // Sent to Nonce Storage (condition: command CMD_NONCE_IDX_READ)
    output logic read_nonce_index_pulse_out,
    // Sent to Nonce Storage (condition: non-command during MODE_NONCE_BYTES_INDEX_SETUP)
    output logic write_nonce_index_pulse_out,
    output types_pkg::chacha_nonce_index_t write_nonce_index_out,
    ...
    // Sent to Key Storage (condition: command CMD_KEY_IDX_READ)
    output logic read_key_index_pulse_out,
    // Sent to Key Storage (condition: non-command during MODE_KEY_BYTES_INDEX_SETUP)
    output logic write_key_index_pulse_out,
    output types_pkg::chacha_key_index_t write_key_index_out,
    ...
    // Sent to Block Counter (condition: command CMD_BLOCK_CNT_IDX_READ)
    output logic read_block_counter_index_pulse_out,
    // Sent to Block Counter (condition: non-command during MODE_BLOCK_COUNTER_INDEX_SETUP)
    output logic write_block_counter_index_pulse_out,
    output types_pkg::chacha_block_counter_index_t write_block_counter_index_out,
  ...
  assign read_nonce_index_pulse_out =
      (command_in && pulse_in && command_code == types_pkg::CMD_NONCE_IDX_READ);
  assign write_nonce_index_pulse_out =
      (!command_in && pulse_in && command_mode == types_pkg::MODE_NONCE_BYTES_INDEX_SETUP);
  assign write_nonce_index_out = types_pkg::chacha_nonce_index_t'(input_byte_in[3:0]);
  ...
  assign read_key_index_pulse_out =
      (command_in && pulse_in && command_code == types_pkg::CMD_KEY_IDX_READ);
  assign write_key_index_pulse_out =
      (!command_in && pulse_in && command_mode == types_pkg::MODE_KEY_BYTES_INDEX_SETUP);
  assign write_key_index_out = types_pkg::chacha_key_index_t'(input_byte_in[4:0]);
  ...
  assign read_block_counter_index_pulse_out =
      (command_in && pulse_in && command_code == types_pkg::CMD_BLOCK_CNT_IDX_READ);
  assign write_block_counter_index_pulse_out =
      (!command_in && pulse_in && command_mode == types_pkg::MODE_BLOCK_COUNTER_INDEX_SETUP);
  assign write_block_counter_index_out = input_byte_in[2:0];
  ...
  assign read_mode_pulse_out =
      (command_in && pulse_in && command_code == types_pkg::CMD_STATUS_READ);
  ...
  always_comb begin
    ...
    if (command_in && pulse_in) begin
      case (command_code)
        ...
        types_pkg::CMD_NONCE_IDX_WRITE: begin
          next_command_mode = types_pkg::MODE_NONCE_BYTES_INDEX_SETUP;
        end
        ...
        types_pkg::CMD_KEY_IDX_WRITE: begin
          next_command_mode = types_pkg::MODE_KEY_BYTES_INDEX_SETUP;
        end
        ...
        types_pkg::CMD_BLOCK_CNT_IDX_WRITE: begin
          next_command_mode = types_pkg::MODE_BLOCK_COUNTER_INDEX_SETUP;
        end
